...
def genStableGalaxy(n_stars, m_star, m_bh):
    ...
    return make_body_array(positions, velocities, masses)
...
result = g.LeapFrog(galaxy, dt=1e12, n_steps=n_steps, G=sc.G)
print(result)
bl = utils.zip_to_bodylist(result['pos'], result['vel'], result['mass'])
result = cs.LeapFrogSaveC(bl, 0, 1)
result.save("gpu.binv")