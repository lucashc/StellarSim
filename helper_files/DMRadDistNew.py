...
def PIradPDF(r, rho0=1, Rc=scale, R_halo=edge):
    ...
    if r < R_halo:
        return rho0/(1+(r/Rc)**2)*4*np.pi*r**2/normalise
    else:
        return 0


def PIradSample(size, rho0=1, Rc=scale, R_halo = edge):
      """Returns n samples from radial distribution described by PIradPDF."""
      samples = []
      while len(samples) < size:
          r = np.random.uniform(low=0, high=R_halo)
          prop = PIradPDF(r, rho0, Rc, R_halo)
          if np.random.uniform(low=0, high=1) <= prop:
              samples += [r]
